...
import { filterCommercialSoundResults } from "@/lib/media/commercial-audio";
	...
	try {
		...
		if (!freesoundValidation.success) {
			...
			if (rawData?.results) {
				...
				const transformedResults = filterCommercialSoundResults(data.results.map((r: Record<string, unknown>) => ({
					id: r.id,
					name: r.name ?? "Unknown",
		...
		const transformedResults = filterCommercialSoundResults(
