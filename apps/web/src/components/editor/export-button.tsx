...
import { getUnverifiedLibraryAudioElements } from "@/lib/media/commercial-audio";
		...
		>
			<PopoverTrigger asChild>
				<button
					type="button"
					className={cn(
						"nexgc-export-trigger flex items-center gap-1.5 rounded-md bg-[#38BDF8] px-[0.12rem] py-[0.12rem] text-white",
...
function ExportPopover({
	...
	const handleExport = async () => {
		...
		if (
			COMMERCIAL_MODE &&
			shouldIncludeAudio &&
			getUnverifiedLibraryAudioElements(editor.timeline.getTracks()).length > 0
		) {
								...
								<div className="p-3 pt-0">
									{hasLicenseError && (
										<p className="mb-2 text-xs text-destructive" role="alert">
											Export blocked: remove or replace library sounds with
											missing or non-CC0 license information.
										</p>
									)}
