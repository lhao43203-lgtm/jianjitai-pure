...
import { TimelineEffectsBadge } from "./timeline-effects-badge";
import { TimelineTransitionBadge } from "@/components/editor/transition-control";
			...
			<ContextMenuContent className="w-64">
				{isVisualElement(element) && element.transitionOut && (
					<ContextMenuItem
						...
						onClick={() =>
							invokeAction("remove-transition", {
								trackId: track.id,
								elementId: element.id,
							})
						}
				...
				{"effects" in element && Boolean(element.effects?.length) && (
					<ContextMenuItem
						onClick={() => {
							selectElement({ trackId: track.id, elementId: element.id });
							openClipEffects({ trackId: track.id, elementId: element.id });
						}}
					>
...
function ElementInner({
	...
	const { t } = useI18n();
	const transition = isVisualElement(element)
	...
	const effectIndicator =
		element.type !== "audio" && element.type !== "effect"
			? getEffectIndicatorState((element as VisualElement).effects)
			: null;
	const boxShadows = [
		isSelected ? `0 0 0 ${ELEMENT_RING_WIDTH_PX}px var(--foreground)` : null,
	...
	return (
		<div
			...
			title={
				[
					effectIndicator
						? `${t("Manage applied effects")} · ${effectIndicator.label}`
						: null,
					transition ? t("Applied transition") : null,
				]
					.filter(Boolean)
					.join(" · ") || undefined
			}
		>
			<div
				...
				style={
					boxShadows.length ? { boxShadow: boxShadows.join(", ") } : undefined
				}
			>
				<button
					type="button"
					id={`timeline-clip-${element.id}`}
				...
				{effectIndicator && (
					<div
						...
						style={{
							boxShadow:
								effectIndicator.kind === "all-disabled"
									? "inset 0 0 0 1px rgb(139 92 246 / 0.5)"
									: "inset 0 0 0 2px rgb(124 58 237)",
						}}
					/>
				)}
				{transition && (
					<div
						aria-hidden="true"
						className="pointer-events-none absolute inset-y-0 right-0 z-10 w-1 bg-cyan-500"
					/>
				)}
			</div>

			{effectIndicator && elementWidth >= 64 && (
				<div className="sticky left-1 mt-1 ml-1 w-fit">
					<TimelineEffectsBadge
						element={element as VisualElement}
						trackId={track.id}
						locked={Boolean(track.locked)}
						compact={elementWidth < 110}
					/>
				</div>
			)}
			...
			{transition &&
				elementWidth >=
					(element.type === "text" && effectIndicator ? 100 : 40) && (
					<div className="absolute bottom-0.5 right-1 z-10 max-w-[calc(100%-8px)]">
						<TimelineTransitionBadge
							...
							compact={
								elementWidth <
								(element.type === "text" && effectIndicator ? 220 : 150)
							}
						/>
					</div>
				)}
	...
	return (
		<div
			...
			onKeyDown={(event) => {
				...
				event.stopPropagation();
				const step = ["ArrowUp", "ArrowRight"].includes(event.key)
					? 0.05
					: -0.05;
				onVolumeCommit(Math.max(0, Math.min(1, volume + step)));
			}}
