...
import { buildLocalAudioElement } from "@/lib/media/local-audio";
		...
		(target) => {
			...
			try {
				const asset = editor.media
					.getAssets()
					.find((item) => item.id === target.mediaId);
				editor.timeline.insertElement({
					element: buildLocalAudioElement(
						asset,
						editor.playback.getCurrentTime(),
					),
					placement: { mode: "auto" },
				});
				toast.success(t("Audio added to timeline"));
			} catch {
				toast.error(
					t("This audio cannot be added. Try importing an MP3 or WAV file."),
				);
			}
		},
		undefined,
	...
	useActionHandler(
		...
		(target) => {
			...
			if (
				...
				return;
			editor.command.execute({ command: new RemoveTransitionCommand(target) });
		},
		undefined,
	);
	useActionHandler(
		...
		(target) => {
			if (
				!target ||
				editor.timeline.getTrackById({ trackId: target.trackId })?.locked
			)
				return;
			editor.timeline.removeClipEffect(target);
		},
		undefined,
	);
	useActionHandler(
		...
		(target) => {
			if (
				...
				return;
			editor.timeline.toggleClipEffect(target);
		},
		undefined,
	);
	useActionHandler(
		...
		(target) => {
			...
			editor.timeline.updateElements({
				updates: [{ ...target, updates: { effects: [] } }],
			});
		},
		undefined,
	);
