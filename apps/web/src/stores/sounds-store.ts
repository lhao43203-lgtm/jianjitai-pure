...
export const useSoundsStore = create<SoundsStore>((set, get) => ({
	...
	addSoundToTimeline: async ({ sound }) => {
		if (
			COMMERCIAL_MODE &&
			!isCommercialSoundLicenseAllowed(sound.license)
		) {
			toast.error("Commercial mode only allows verified CC0 library sounds");
			return false;
		}
		...
		try {
			...
			const element = buildLibraryAudioElement({
				...
				buffer,
				sourceLicense: sound.license,
				sourceCreator: sound.username,
